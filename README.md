...
一个简洁高效的在线工具集合，包含文本对比、JSON 格式化、生活账单、今日时间管理、买前查价等实用工具。
...
- 统计入口：`/analytics/`
- 采集范围：网站首页和各工具首页，包括 `/tools/diff/`、`/tools/json/`、`/tools/expense/`、`/tools/time/`、`/tools/price/`
...
### 5. 🧾 真价助手
一个本地优先的买前查价 PWA，用本地历史价格和分位区间辅助判断当前价格是否真低。

**访问路径**: `/tools/price/`

#### 功能特性
- 🔗 **商品链接解析** - 支持京东、淘宝、天猫、拼多多商品详情页链接识别
- 📉 **价格曲线** - 基于本地记录展示历史价格走势
- 🧠 **真低价判断** - 使用历史最低价和近 90 天分位数给出可信评分、原因和建议
- ✍️ **手动记录** - 支持记录当前价、促销备注和本地观察数据
- 🎯 **本地关注** - 支持设置目标价并维护关注清单
- 📦 **导出导入** - 支持 JSON 备份和恢复，数据保存在本地浏览器
- 📱 **PWA 体验** - 支持独立安装、离线静态资源和移动端布局
...
### 2026-07
- 🧾 新增真价助手买前查价 PWA，支持商品链接解析、本地价格曲线、真低价判断、关注目标价和 JSON 备份。
